    ...
    <el-table style="width: 100%" :data="messages">
      ...
      <el-table-column label="操作" fixed="right" align="center" width="180">
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="$router.push(`/message/edit/${scope.row.id}`)"
            >编辑</el-button
          >
...
export default {
  ...
  methods: {
    async fetch() {
      ...
      this.messages = res.data.slice(0, this.pageSize);
    },
    async remove(row) {
    ...
    async handleSizeChange(val) {
      ...
      const res = await this.$http.get(
        `/messages/get/page?pageSize=${this.pageSize}&currentPage=${this.currentPage}`
      );
    ...
    async search() {
      const res = await this.$http.post("/messages/get/search", this.form);
      this.messages = res.data;
      this.count = res.data.length;
